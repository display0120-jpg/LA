...
# --- [2. AI 모델 강제 고정 (Gemini 1.5 Flash)] ---
...
# 하루 1,500회 무료인 안정적인 1.5 모델을 사용합니다.
# 절대 gemini-2.5-flash 라고 적지 마세요.
        ...
        if st.button("AI 가이드 받기 💡"):
            with st.spinner("AI 분석 중..."):
                try:
                    ...
                    if "404" in str(e):
                        st.error("❌ 에러: 모델을 찾을 수 없습니다. requirements.txt 라이브러리 버전을 확인하세요.")
                    elif "429" in str(e):
                        st.error("🚨 한도 초과: 1분만 기다렸다가 다시 해주세요.")
                    else:
                        st.error(f"오류 발생: {e}")
...
st.markdown("---")
st.caption("대지고등학교 환경 프로젝트 | Powered by Gemini 1.5 Flash (Stable)")
