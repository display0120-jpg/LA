    ...
    if img1:
        if st.button("AI 가이드 받기 💡"):
            with st.spinner("분석 중..."):
                try:
                    # 1.5 Flash는 한도가 넉넉하여 에러가 거의 나지 않습니다.
                    res = model.generate_content(["이 쓰레기 분리배출법을 딱 3줄 요약해줘. 1.비움 2.제거 3.분류 형식!", Image.open(img1)])
                    st.session_state.guide = res.text
                    st.session_state.step = 2
                    st.rerun()
                except Exception as e:
                    if "429" in str(e):
                        st.error("🚨 잠시만요! 사용자가 너무 많아 1분만 기다렸다가 다시 시도해 주세요.")
                    else:
                        st.error(f"분석 실패: {e}")
    ...
    if img2 and not st.session_state.verified:
        if st.button("최종 인증하기 ✅"):
            with st.spinner("검사 중..."):
                try:
                    verify_prompt = f"가이드: {st.session_state.guide}. 사진을 보고 미션을 잘 했는지 확인해줘. 성공하면 무조건 '인증성공'이라는 단어를 포함해서 칭찬해줘."
...
st.markdown("---")
st.caption("대지고등학교 환경 프로젝트 | 모델: Gemini 1.5 Flash (안정 버전)")
